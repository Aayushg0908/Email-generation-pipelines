...
LOG_FILE = "E:\\Synthetic-Email-Generation-tool-main\\client-feedback\\client_feedback_history.log"  # <--- NEW LOG FILE
NUM_EMAILS = 700       
DELAY_SECONDS = 0.1      
MAX_RETRIES = 3          
...
BASE_USER_PROMPT = """Generate a realistic client feedback update email sent FROM a recruitment firm or company TO a candidate.
This email is sent after feedback has been received from the external client regarding the candidate profile.
...
- Inform the candidate that feedback has been received from the client
- Thank the candidate for their patience during the review process
- State that the feedback will be carefully considered
- Indicate that next steps will be communicated shortly
- Do NOT confirm selection, rejection, or interview scheduling
- Do NOT include specific feedback details
...
SUBJECT: Client Feedback Update | UX/UI Designer Position

BODY:
Dear Rohan Malhotra,

We would like to inform you that we have received feedback from the client regarding your profile for the UX/UI Designer position.

Thank you for your patience while the review process was ongoing. The feedback is currently being evaluated by our recruitment team, and we will share further updates with you shortly.
...
TalentSphere Solutions

TONE: Professional, neutral, and informative

INTENT: Inform candidate that client feedback has been received and next steps will follow
