...
OUTPUT_CSV = "E:\\Synthetic-Email-Generation-tool-main\\Candidate_availability\\candidate_availability.csv"
DEBUG_DIR = "E:\\Synthetic-Email-Generation-tool-main\\Candidate_availability\\debug"
LOG_FILE = "E:\\Synthetic-Email-Generation-tool-main\\Candidate_availability\\candidate_availability_history.log"  # <--- NEW LOG FILE
NUM_EMAILS = 600       
DELAY_SECONDS = 0.1      
MAX_RETRIES = 3          
...
BASE_USER_PROMPT = """Generate a realistic interview scheduling acknowledgment email sent FROM a company TO a candidate.
This email is sent after the candidate has shared their availability for an interview.

CONTEXT:
- The interview is for a {role} position
...
- Thank the candidate for sharing their availability
- Clearly state that the interview will be scheduled soon
- Mention that interview details will be shared shortly
- Professional, neutral, ATS-style tone
- Short, realistic corporate email
- No double quotes anywhere
- Do NOT confirm a specific interview date or time
...
SUBJECT: Availability Received – Interview Scheduling in Progress | Business Analyst Role

BODY:
Dear Alex Morgan,

Thank you for providing your availability for the Business Analyst position at Horizon Consulting.

Our recruitment team is currently reviewing the schedules shared by candidates. Based on your background as a motivated professional, we will coordinate the next steps accordingly.

We will be scheduling your interview soon and will share the interview details with you shortly.

Thank you for your continued interest in Horizon Consulting.
...
INTENT: Acknowledge receipt of candidate availability
