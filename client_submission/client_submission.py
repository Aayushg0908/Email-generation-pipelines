...
OUTPUT_CSV = "E:\\Synthetic-Email-Generation-tool-main\\client_submission\\client_submission.csv"
DEBUG_DIR = "E:\\Synthetic-Email-Generation-tool-main\\client_submission\\debug"
LOG_FILE = "E:\\Synthetic-Email-Generation-tool-main\\client_submission\\client_submission_history.log"  # <--- NEW LOG FILE
NUM_EMAILS = 600       
DELAY_SECONDS = 0.1      
MAX_RETRIES = 3          
...
BASE_USER_PROMPT = """Generate a realistic client submission update email sent FROM a recruitment firm or company TO a candidate.
This email is sent after the candidate profile has been submitted to an external client for review.

CONTEXT:
- The role submitted is {role}
- The candidate described themselves as {adjective}

REQUIREMENTS:
- Invent realistic fictional names for the candidate, company, and recruiter or HR representative
- The candidate name and recruiter or HR representative name MUST be different in every email
- Do NOT reuse the same recruiter or candidate name across emails
- Inform the candidate that their profile has been successfully submitted to the client
- Clearly state that client feedback is pending
- Assure the candidate that updates will be shared once feedback is received
- Do NOT confirm selection, rejection, or interview scheduling
- Do NOT mention client identity unless fictional
- Professional, informative, ATS-style tone
...
SUBJECT: Profile Submitted to Client | Project Manager Position

BODY:
Dear Ananya Sharma,

We would like to inform you that your profile has been successfully submitted to our client for the Project Manager position.

The client is currently reviewing the submitted profiles, and feedback is awaited. We will keep you informed as soon as we receive any updates from the client regarding the next steps.
...
INTENT: Inform candidate of client submission and indicate feedback will follow
